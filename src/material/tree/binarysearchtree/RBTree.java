...
public class RBTree<E> implements BinarySearchTree<E> {
     ...
     * @param <T>
     */
    private static class RBInfo<T> implements Comparable<RBInfo<T>>, Position<T> {
    ...
    private static class RBTreeIterator<T> implements Iterator<Position<T>> {
    ...
    public E remove(Position<E> posV) throws RuntimeException {
        ...
        if (formerPosition != nodeV) {
            ...
            // root must be black
            formerPosition.getElement().setColor(VisRed != this.bst.binTree.isRoot(formerPosition));
        }
